    ...
    layui.use(['laypage', 'layer', 'form'], function () {
        ...
        $.ajax({
            ...
            success: function(data) {
            	 //请求出错处理
                /*alert('获取总页数失败');*/
                pages = data.postList1.totalRecord%10==0?data.postList1.totalRecord/10:(data.postList1.totalRecord/10+1);
                laypage({
                    cont: 'page'
                    ,pages:pages //得到总页数
                    ,skip:true
        ...
        function ajaxPageContent(data) {
            ...
            $tbody.html("");
            var my_length = data.postList1.dataList.length;
            $.each(data.postList1.dataList,function (index,element) {
                var $tr = $('<tr></tr>');
                ...
                success:function () {
                    $.ajax({
                        ...
                        success: function(data) {
                             ...
                             $("#layui-layer-iframe1").contents().find("#forum_replay").val(data.reply);
                             $("#layui-layer-iframe1").contents().find("#forum_reply_form").on('submit',function (e) {
                            	 console.log('dianjilehuifu')
                            	 var username =  $("#layui-layer-iframe1").contents().find("#forum_username").val();
                            	 var content = $("#layui-layer-iframe1").contents().find("#forum_content").html();
                                 var reply= $("#layui-layer-iframe1").contents().find("#forum_replay").val();
                                 console.log(reply)
                                 $.ajax({
                                     url: "../postupPKS",    //请求的url地址
                                     dataType: "json",   //返回格式为json
                                     async: true, //请求是否异步，默认为异步，这也是ajax重要特性
                                     data: { "id": id,'title':data.titile,'content':data.content,'release_time':data.releaseTime,'reply':reply},    //参数值 可能不用
                                     type: "POST",   //请求方式
                                     success: function(data) {
                                    	 //请求出错处理
                                         /*alert('获取总页数失败');*/
                                         layer.alert("回复帖子成功", {icon: 6},function () {
                                             // 获得frame索引
                                             //关闭当前frame
