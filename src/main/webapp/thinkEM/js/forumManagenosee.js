    ...
    layui.use(['laypage', 'layer', 'form'], function () {
        ...
        $.ajax({
            ...
            success: function(data) {
                //请求出错处理
                /*alert('获取总页数失败');*/
                pages = data.postList0.totalRecord%10==0?data.postList0.totalRecord/10:(data.postList0.totalRecord/10+1);
                laypage({
                    cont: 'page'
                    ,pages:pages //得到总页数
                    ,skip:true
        ...
        function ajaxPageContent(data) {
            ...
            $tbody.html("");
            var my_length = data.postList0.dataList.length;
            $.each(data.postList0.dataList,function (index,element) {
                var $tr = $('<tr></tr>');
                ...
                success:function () {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            $("#layui-layer-iframe1").contents().find("#forum_pass").on('click',function (e) {
                                $.ajax({
                                    ...
                                    success: function(data) {
                                        //请求出错处理
                                        /*alert('获取总页数失败');*/
                                        layer.alert("通过帖子成功", {icon: 6},function () {
                                            // 获得frame索引
                                            //关闭当前frame
                            ...
                            $("#layui-layer-iframe1").contents().find("#forum_nopass").on('click',function (e) {
                                $.ajax({
                                    url: "../postupPKS",    //请求的url地址
                                    dataType: "json",   //返回格式为json
                                    async: true, //请求是否异步，默认为异步，这也是ajax重要特性
                                    ...
                                    success: function(data) {
                                        //请求出错处理
                                        /*alert('获取总页数失败');*/
                                        layer.alert("不通过帖子成功", {icon: 6},function () {
                                            // 获得frame索引
                                            //关闭当前frame
                                            layer.closeAll('iframe');
                                        });
